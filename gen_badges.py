...
def check_is_changed(conn, cur, package_name, package_ver):
...
def gen_badge(conn, cur, badge_text_pack, db_type='MySQL', app_name='app_name', app_ver='0.0.0', app_lm='2025-01-02 09:18:18'):
    ...
    # 生成app版本号badge
    if check_is_changed(conn, cur, app_name, app_ver):
    ...
    if check_is_changed(conn, cur, 'Python', python_ver):
    ...
    if db_type == 'MySQL':
        ...
        db_ver = 'unknown'
    if check_is_changed(conn, cur, db_type, db_ver):
    ...
    for package in badge_text_pack:
        ...
        if check_is_changed(conn, cur, package, package_version):
