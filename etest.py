...
def display_weather(city_code):
    ...
    if weather_info:
        ...
        wcol[0].metric('天气', f"{weather_info['weather']} {weather_info['weather_icon']}")
        wcol[1].metric('温度', f"{weather_info['temperature']} ℃ {weather_info['temp_icon']}")
        wcol[2].metric('风向', f"{weather_info['winddirection']}风")
        wcol[3].metric('风力', f"{weather_info['windpower']}km/s {weather_info['wind_icon']}")
        wcol[4].metric('湿度', f"{weather_info['humidity']}% {weather_info['humidity_icon']}")
        # 设置度量卡片的样式
        style_metric_cards(border_left_color="#8581d9")
